...
class Farneback3D:
    ...
    def calculate_flow(self, image1: np.ndarray, image2: np.ndarray,
        ...
        if np.any(total_vol > sub_volume):
            ...
            toc = time.time()
            print(
            f"Iters: {self.iters} | Levels: {self.num_levels} | Scale: {self.scale} | Kernel: {self.spatial_size} | Filter: {self.filter_type}-{self.filter_size} | Presmoothing: {self.presmoothing} | Time: {toc - tic:.4f}s | {image1.dtype}",
            flush=True)
